...
class DummyExtraFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        return diffusion_utils.PlaceHolder(X=empty_x, E=empty_e, y=empty_y)
...
class ExtraFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        #print(y_cycles.shape)  torch.Size([bs, 4])
        
        device=noisy_data['E_t'].device
        n = torch.ones(x_cycles.size(0), 1).to(device)
        ...
        if self.features_type == 'cycles':
            ...
            return diffusion_utils.PlaceHolder(X=x_cycles, E=extra_edge_attr, y=torch.hstack((n, y_cycles)))
            ...
            return diffusion_utils.PlaceHolder(X=x_cycles, E=extra_edge_attr, y=torch.hstack((n, y_cycles, n_components,
                                                                                    batched_eigenvalues)))
            ...
            n_components, batched_eigenvalues, nonlcc_indicator, k_lowest_eigvec = eigenfeatures   # (bs, 1), (bs, 10),
                                                                                                   # (bs, n, 1), (bs, n, 2)
            #print(n.shape, y_cycles.shape, n_components.shape, batched_eigenvalues.shape)
            return diffusion_utils.PlaceHolder(X=torch.cat((x_cycles, nonlcc_indicator, k_lowest_eigvec), dim=-1),
                                     E=extra_edge_attr,
...
class EigenFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        if self.mode == 'eigenvalues':
            ...
            eigvals = eigvals.type_as(A) / torch.full(A.shape[0], 116)
...
def get_eigenvectors_features(vectors, n_connected, device, k=2):
    """
    ...
    bs, n = first_ev.shape
    node_mask = torch.full((bs, n), True, dtype=torch.bool, device=device)
...
class KNodeCycles:
    ...
    def calculate_kpowers(self):
        self.k1_matrix = self.adj_matrix.float() #A
        self.d = self.adj_matrix.sum(dim=-1)
        self.k2_matrix = self.k1_matrix @ self.adj_matrix.float()  #A2
        self.k3_matrix = self.k2_matrix @ self.adj_matrix.float()  #A3
        self.k4_matrix = self.k3_matrix @ self.adj_matrix.float()  #A4
        self.k5_matrix = self.k4_matrix @ self.adj_matrix.float()  #A5
        self.k6_matrix = self.k5_matrix @ self.adj_matrix.float()  #A6

    def k3_cycle(self):
        """ tr(A ** 3). """
        c3 = batch_diagonal(self.k3_matrix)  
        return (c3 / 2).unsqueeze(-1).float(), (torch.sum(c3, dim=-1) / 6).unsqueeze(-1).float()  #X3, y3 

    def k4_cycle(self):
        diag_a4 = batch_diagonal(self.k4_matrix)
        c4 = diag_a4 - self.d * (self.d - 1) - (self.adj_matrix @ self.d.unsqueeze(-1)).sum(dim=-1)
        return (c4 / 2).unsqueeze(-1).float(), (torch.sum(c4, dim=-1) / 8).unsqueeze(-1).float()  #X4, y4
    ...
    def k5_cycle(self):
        ...
        return (c5 / 2).unsqueeze(-1).float(), (c5.sum(dim=-1) / 10).unsqueeze(-1).float()   ##X5, y5
    ...
    def k6_cycle(self):
        ...
        return None, (c6_t / 12).unsqueeze(-1).float()  #obtain y6
