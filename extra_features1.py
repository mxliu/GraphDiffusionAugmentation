...
class ExtraFeatures1:
    ...
    def __call__(self, X,E,y):
        x_cycles, y_cycles = self.ncycles(X,E,y)       # (bs, n_cycles)
        n = torch.ones(x_cycles.size(0), 1)
        ...
        if self.features_type == 'cycles':
            ...
            eigenfeatures = self.eigenfeatures(X,E,y)
...
def get_eigenvectors_features(vectors, n_connected, k=2):
    ...
    bs, n = first_ev.shape
    node_mask = torch.full((bs, n), True, dtype=torch.bool, device=first_ev.device)
